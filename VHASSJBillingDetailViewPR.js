    ...
    define("siebel/custom/VHASSJBillingDetailViewPR", ["siebel/viewpr", "siebel/custom/VHASSJValidations"],
        ...
        SiebelAppFacade.VHASSJBillingDetailViewPR = (function () {
            ...
            VHASSJBillingDetailViewPR.prototype.ShowUI = function () {
                ...
                if(sClick=="New"){
                    if(SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'] != null)
                        {
                            var sBiALA = SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'].GetFullId();
                            if(sBiALA)
                            {
                                $("#" + sBiALA).addClass("VFDisplayNone");
                                $('.vhassjeditbilldetails').addClass('VFDisplayNone');
                                
                            }
                        }
                }
                if(sClick=="Cancel" || sClick=="Save"){
                    if(SiebelApp.S_App.GetActiveView().GetAppletMap()['VHA SSJ Billing Account Address List Applet TBUI'] != null)
                    {
                        var sBiALstApplet = SiebelApp.S_App.GetActiveView().GetAppletMap()['VHA SSJ Billing Account Address List Applet TBUI'].GetFullId();
                        if(sBiALstApplet)
                        {
                            $("#" + sBiALstApplet).addClass("VFDisplayNone");
                            
                        }
                    }
                }
                if(sClick=="BillAddrEdit")
                {
                    $('.vhassjeditbilldetails').addClass('VFDisplayNone');
                    $('.vhassjeditbillbtn button').addClass('VFDisplayNone');
                }
                //CM-11200- Start code added by samala - For DFA_Billing Profile Legacy screen //
                setTimeout(() => {
                      var sView1 = SiebelApp.S_App.GetActiveView().GetName();
                      var isDFAFlow = SiebelApp.S_App.GetProfileAttr("VHANewOrg");
                      var sApplet = SiebelApp.S_App.GetActiveView().GetActiveApplet().GetName()
                      if (
                        sView1 === "VF Billing Account Profile View"  &&
                      (isDFAFlow === "TPG" || isDFAFlow === "iiNet")){
                      if (sApplet  === "DFA  Com Invoice Profile Form Toggle Applet - Personal AU" ){
                        $(".siebui-icon-icon_required").addClass('VFLFDisplayNone');
                        $(".siebui-icon-undorecord").addClass('refreshiconqurey'); 
                      }
                        if (SiebelApp.S_App.GetActiveView().GetAppletMap()['VHA DFA Billing Setup Applet TBUI']){
                        $(".SFJHeading").addClass('VFLFDisplayNone');
                        $(".paymentterm").addClass('VFLFDisplayNone');
                        
                      }
                      }
                      }, 500);
                //CM-11200- END code added by samala - For DFA_Billing Profile Legacy screen //
            }
            ...
            VHASSJBillingDetailViewPR.prototype.BindEvents = function () {
                ...
                $(document).off("click.vhaSSJLayout").on("click.vhaSSJLayout", "button, a", function () {
                    ...
                    if (txt == "edit" || txt == "save" || txt == "discard") {
                        ...
                        var targetContainer = $(".BillingAddressContainer");
                        setTimeout(function () {
                            refreshBillingLayout();
                            // Keep restoring scroll for few seconds
                            var isAddressButton = $btn.closest(".BillingAddressContainer").length > 0 || 
                                                  ($btn.attr("title") && $btn.attr("title").indexOf("Billing Address") !== -1);
                            if (isAddressButton) {
                                var counter = 0;
                                var scrollFix = setInterval(function () {
                                    if (targetContainer.length) {
                                        $("html, body").scrollTop(
                                            targetContainer.offset().top - 150
                                        );
                                    }
                                    // Focus editable field
                                    var firstField = $(".BillingAddressContainer").find("input, select, textarea").filter(":visible:first");
                ...
                $(document).on("click", 'button[title="Captured billing addresses List Applet:New"]', function () {
                    sClick = 'New';

                });
                
                $(document).on("click", 'button[title="Captured billing addresses List Applet:Cancel"]', function () {
                    sClick = 'Cancel';
                    /*if(SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'] != null)
                    {
                        var sBiALA2 = SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'].GetFullId();
                        if(sBiALA2)
                        {
                            $("#" + sBiALA2).removeClass("VFDisplayNone");
                            //$('.vhassjeditbilldetails').addClass('VFDisplayNone');
                ...
                $(document).on("click", 'button[title="Captured billing addresses List Applet:Save"]', function () {
                    sClick = 'Save';
                    /*if(SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'] != null)
                    {
                        var sBiALA1 = SiebelApp.S_App.GetActiveView().GetAppletMap()['VF SSJ Billing Account Address Details TBUI'].GetFullId();
                        if(sBiALA1)
                        {
                            $("#" + sBiALA1).removeClass("VFDisplayNone");
                            //$('.vhassjeditbilldetails').addClass('VFDisplayNone');
                            
                        }
                    }*/
                });
                 $(document).on("click", 'button[title="Billing Address Form Applet:Edit"]', function () {
                    sClick = 'BillAddrEdit';
                });
            }
